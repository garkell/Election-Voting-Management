...
class GavotingViewNomination extends BaseHtmlView
{
	...
	protected function addToolbar()
	{
		...
		ToolbarHelper::title(Text::_('COM_GAVOTING_TITLE_NOMINATION'), 'nomination.png');

		// Build the actions for new and existing records.
		if ($isNew) {
			// For new records, check the create permission.
			if ($isNew && ($user->authorise('core.create'))) {
				ToolbarHelper::apply('nomination.apply');

				ToolbarHelper::saveGroup(
					[
						['save', 'nomination.save'],
						['save2new', 'nomination.save2new']
					],
					'btn-success'
				);
			}

			ToolbarHelper::cancel('nomination.cancel');
		} else {
			...
			if (!$checkedOut && $itemEditable) {
				...
				$toolbarButtons[] = ['save', 'nomination.save'];

				// We can save this record, but check the create permission to see if we can return to make a new one.
				if ($canDo->get('core.create')) {
					$toolbarButtons[] = ['save2new', 'nomination.save2new'];
				}
			}

			// If checked out, we can still save
			if ($canDo->get('core.create')) {
				$toolbarButtons[] = ['save2copy', 'nomination.save2copy'];
			}
