...
class GaupdparamsHelper
{
	...
	{
		$lastYear = (int) GavotingHelper::getElectionYear();
		$nxtYear = $lastYear + 1;
	...
	public static function updatePositions($id = 0)
	{
		...
		foreach ($objects AS $obj) {
			...
			$newObj->cat_id = $id;
			//$newObj->elect_date = '0000-00-00 00:00:00';
	...
	{
		$db		= Factory::getDbo();
		$db->setQuery((string)'SELECT nom_name, max(votes) AS vote_count FROM #__gavoting_nominations WHERE state = 1 AND position_id = '.(int) $id.' GROUP BY nom_name LIMIT 1' );
	    try {
    ...
    public static function updateExtensionParams($nxtYear = '2020')
	{
        ...
        $nxtAGM = $nxtYear.substr($params->agm_date,4,6);
        $clNoms = $nxtYear.substr($params->close_noms,4,6);
        $opVote = $nxtYear.substr($params->open_votes,4,6);
        $clVote = $nxtYear.substr($params->close_votes,4,6);
